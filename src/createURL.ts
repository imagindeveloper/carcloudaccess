...
export const createURL = (baseUrl: string, params:any= <any>{}, angleOverride: string) => {
  let carcloudFileType = (params.hasOwnProperty('filetype') && params.filetype != 0 ? params.filetype : 'png');
  if (carcloudFileType !== 'jpg' || carcloudFileType !== 'png') {
    carcloudFileType = 'png';
  }
  const carcloudCompare = (params.hasOwnProperty('compare') && params.compare != 0 ? params.compare : 'original');
  ...
  const carcloudInterior = (params.hasOwnProperty('interior') ? params.interior : 0);
  const carcloudBodyColorId = (params.hasOwnProperty('bodycolor') ? params.bodycolor : 0);
  const carcloudRoofColorId = (params.hasOwnProperty('roofcolor') ? params.roofcolor : 0);
  const carcloudRimId = (params.hasOwnProperty('rimid') ? params.rimid : 0);
  let carcloudAngle = (typeof angleOverride !== 'undefined' ? angleOverride : (params.hasOwnProperty('angle') ? params.angle : (params.getexterior ? '01' : (params.getinterior ? '41' : (params.getrim ? '51' : '01')))));
  if (!carcloudAngle) {
    carcloudAngle = '01';
  }
  let carcloudLayers = '';
